...
(function() {
	...
	iHTML.prototype.appendHTML = function(html){this.html += html;};
	
	//Spit out the object as json
	iHTML.prototype.toJSON = function(){
	    return({
	        "html":this.html, "events":this.events
	    });
	};
					...
					if( prop.length > 2 )
						if( prop.substring(0,2).toLowerCase() === "on" ) {
							...
							if(options.output === "ihtml") {
								...
								var data = {
									"obj":obj, "data":options.data, "index":index
								};
    ...
    function _component(pobj, obj, template, options, index) {
        ...
        var ihtml = new iHTML();
        var component = {
            "template":undefined, "name":undefined
        };
